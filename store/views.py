        ...
        if processor == "paypal":
            ...
            if status:
                ...
                return redirect('order_error',
                                context={"message": "there was a problem making a payment.repo man is comming for you"})
    else:
        return redirect('index')
...
def checkout_stripe(cart, orders, token):
    stripe.api_key = "sk_test_3aswbA9kLuPS81XzPzj8mNDI"
    total = 0
    for order in orders:
        total += (order.book.price * order.quantity)
    status = True
